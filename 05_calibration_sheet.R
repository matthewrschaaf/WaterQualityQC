...
generate_calibration_sheet <- function(results_df, analytes_df, user_input) {
  ...
  full_calibration_sheet <- purrr::map_df(1:nrow(sampling_days), function(i) {
    ...
    sc_cal_date <- if (grepl("00094", collected_storets)) {
      valid_dates <- weekly_dates %>% filter(Weekly_Cal_Date <= trip$Sample_Date & Weekly_Cal_Date >= (trip$Sample_Date - 7))
    ...
    do_cal_date <- if (grepl("00299", collected_storets) || grepl("00301", collected_storets)) {
      valid_dates <- do_dates %>% filter(DO_Cal_Date == trip$Sample_Date)
      if (nrow(valid_dates) > 0) top_n(valid_dates, 1, DO_Cal_Date)$DO_Cal_Date else NA
    } else { NA }
    
    orp_cal_date <- if (grepl("00090", collected_storets)) {
      valid_dates <- orp_turb_dates %>% filter(ORP_Turb_Cal_Date <= trip$Sample_Date & ORP_Turb_Cal_Date >= (trip$Sample_Date - 60))
    ...
    chl_cal_date <- if (grepl("32241", collected_storets) || grepl("32244", collected_storets)) {
      valid_dates <- chl_pc_dates %>% filter(Chl_PC_Cal_Date <= trip$Sample_Date & Chl_PC_Cal_Date >= (trip$Sample_Date - 60))
  ...
  # This logic now correctly happens AFTER the full sheet is created.
  ...
  message("-> Final calibration sheet populated with ", nrow(calibration_sheet), " rows.")
  message("-> Lake Profiles sheet populated with ", nrow(lake_profiles_and_misc), " rows.")
